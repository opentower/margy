...
- `system_mail_watcher.sh` notifies the administrators when local email is received, which usually only happens if an email failed to send (NB: This is the only system information that is not anonymous; if MARGY tries and fails to send an email, the administrators will be alerted and will see what email addresses were involved)
